...
def load_feature_r1():
    if MOCK_DS:
        ...
        #lj
        df1 = pd.read_csv("/home/admin/dev/examples/data/perfect_logit_a.csv")
        y = pd.DataFrame(df1['y']).to_numpy()
        x = pd.DataFrame(df1.iloc[:,:10])
        return x, y
    else:
...
def load_feature_r2():
    if MOCK_DS:
        # x = np.random.rand(MOCK_ROWS, MOCK_COLS)
        #lj
        x = pd.read_csv("/home/admin/dev/examples/data/perfect_logit_b.csv")
        return x
    else:
        from sklearn.datasets import load_breast_cancer
