...
std::pair<std::vector<ArrayRef>, std::vector<ArrayRef>> reconstruct(
  ...
  for (size_t rank = 0; rank < seeds.size(); rank++) {
    // std::cout<<"----- Party "<<rank<<" ---------"<<std::endl;
    for (size_t idx = 0; idx < descs.size(); idx++) {
      // std::cout<<"----- The "<<idx<<" reconstruct------"<<std::endl;
      auto t = prgReplayArray(seeds[rank], descs[idx]);
      // auto t = prgReplayArray(10, descs[idx]);
...
std::tuple<ArrayRef, ArrayRef, ArrayRef, ArrayRef, ArrayRef> TrustedParty::adjustLR(absl::Span<const PrgArrayDesc> descs, 
  ...
  for(i = 0; i < M * N; i++) {
    ...
    // ArrayRef c = ring_ones(descs[0].field, 1);
    for(j = 0; j < K * N; j++) {
      ArrayRef d = rs[1].slice(j, j+1);
      vec_c2.push_back(ring_mul(c, d));
      // ArrayRef d = ring_ones(descs[0].field, 1);
    }
  }
  auto c2 = SimdTrait<ArrayRef>::pack(vec_c2.begin(), vec_c2.end(), pi);

  // std::cout<<"----M, N, K----"<<std::endl;
  // std::cout<<M<<std::endl;
  // std::cout<<N<<std::endl;
  // std::cout<<K<<std::endl;
  // rs[1] = ring_ones(descs[0].field, K * N);
  // r1T = ring_ones(descs[0].field, M * N);
  // auto c2 = ring_mmul(rs[1], r1T, N, M * N, K);

  // std::cout<<"-----c2.numel, elsize, stride, offset---"<<std::endl;
  // std::cout<<c2.numel()<<std::endl;
  // std::cout<<c2.elsize()<<std::endl;
  // std::cout<<c2.stride()<<std::endl;
  // std::cout<<c2.offset()<<std::endl;

  // std::cout<<"-----select element successfully!-----"<<std::endl;
