...
class ArrayRef {
 ...
 public:
  ...
  //Set element.
  void setData(int64_t pos, ArrayRef &value) {
    const char* src_itr = reinterpret_cast<char*>(buf_->data<std::byte>() + stride_ * pos * elsize());
    char* dst_itr = static_cast<char*>(value.data());

    // std::cout<<"----before----"<<std::endl;
    // std::cout<<&src_itr<<std::endl;
    // std::cout<<&dst_itr<<std::endl;

    std::memcpy(&dst_itr, &src_itr, sizeof(src_itr));
