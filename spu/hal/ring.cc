...
Value _mmul(HalContext* ctx, const Value& x, const Value& y) {
  if (x.isPublic() && y.isPublic()) {
    std::cout<<"----------ring.cc : _mmul, x & y public-----------"<<std::endl;
    return _mmul_pp(ctx, x, y);
  } else if (x.isSecret() && y.isPublic()) {
    std::cout<<"----------ring.cc : _mmul, x secret & y public-----------"<<std::endl;
    return _mmul_sp(ctx, x, y);
  } else if (x.isPublic() && y.isSecret()) {
    std::cout<<"----------ring.cc : _mmul, x public & y secret-----------"<<std::endl;
    return transpose(ctx, _mmul_sp(ctx, transpose(ctx, y), transpose(ctx, x)));
  } else if (x.isSecret() && y.isSecret()) {
    // std::cout<<"----------ring.cc : _mmul, x secret & y secret-----------"<<std::endl;
...
Value _logreg(HalContext* ctx, const Value& x, const Value& w, const Value& y) {
  if (x.isPublic() && w.isPublic() && y.isPublic()) {
    std::cout<<"--------ring.cc : _logreg ppp----------"<<std::endl;
    return _logreg_pp(ctx, x, w, y);
  } else{
    // std::cout<<"--------ring.cc : _logreg sss----------"<<std::endl;
...
Value _trunc(HalContext* ctx, const Value& x, size_t bits) {
  ...
  if (x.isPublic()) {
    // std::cout<<"-----ring.cc : _trunc, public-----"<<std::endl;
    return _arshift_p(ctx, x, bits);
  } else if (x.isSecret()) {
    // std::cout<<"-----ring.cc : _trunc, secret-----"<<std::endl;
